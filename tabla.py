...
def hallar_reduccion (Conjunto, sig, reglas, Lista):
    for i in Conjunto:
        if (indice_punto(i.derivada) >= len(i.derivada)):
            copia = []
...
def imprimirG(listaP,listaT,listaNT):
    ...
    for row in range(len(listaP)+2):
        for column in range(len(listaU)+1):
            if row==0:
                if (column ==0):
                    ...
                    label.config(font=('Arial',12))
                    label.grid(row=row,column=column,sticky="nsew",padx=1,pady=1,columnspan=len(listaT)-3)
                    tab1.grid_columnconfigure(column,weight=1)
...
def txt():
    def abrir():
        ...
        print (terminales)
        listau = []
        listau.append(Reglas)
        listau.append(Reglas2)
        listau.append(Ntermin)
        listau.append(terminales)
